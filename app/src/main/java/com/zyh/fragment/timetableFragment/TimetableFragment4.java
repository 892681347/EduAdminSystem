...
public class TimetableFragment4 extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if(!((TimetableFragment) timetableFragment).isFinished[4]){
            Utills.postTimetable(loginBean,timetableFragment,((TimetableFragment) timetableFragment).semester,"4");
        }
        showTimetable();
        return view;
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                getActivity().runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        analyseCourseList(getActivity(),((TimetableFragment) timetableFragment).courseLists.get(4));
                    }
                });
            }
        }).start();
